...
- MetaEditor build 6063: clean compilation. See `compile.log` for the final
  elapsed time and `checksums/ea.sha256` for source/binary identity.
- Compiled target: X64 Regular.
- The SHA-256 values are recorded in `checksums/ea.sha256`.
...
`src/HfmPreflight.mq5` was added to close the broker-evidence gap once
...
- `scripts/telegram_relay.py` passed Python bytecode compilation and a two-event dry run.
...
- All four HFM tester reruns used the v1.34 binary recorded in
  `checksums/ea.sha256`; all reported `Test passed`.
